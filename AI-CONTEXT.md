...
> Référence rapide destinée aux agents IA intervenant sur ce dépôt. Lire ce fichier avant toute modification. Les observations ci-dessous correspondent à la version `0.8.1`. En cas de divergence, la version de `package.json`, le code et les JSON du dépôt priment sur ce document.
...
- `src/pages/`
  ...
  - `PrintBuildPage.tsx` compose une fiche depuis le build courant ou un build sauvegardé. Le paramètre `buildId` recharge un build du catalogue local après actualisation ; sans identifiant, la page se replie sur `lastBuild_v2`.
